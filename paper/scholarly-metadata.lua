...
--- Resolve affiliation placeholders to full named objects
local function resolve_institutes (affiliation, known_institutes)
  local unresolved_institutes
  if affiliation == nil then
    unresolved_institutes = {}
  elseif type(affiliation) == "string" or type(affiliation) == "number" then
    unresolved_institutes = {affiliation}
  else
    unresolved_institutes = affiliation
  end
...
local function canonicalize(raw_author, raw_institute)
  ...
  for _, author in ipairs(authors) do
    author.affiliation = resolve_institutes(
      ensure_list(author.affiliation),
      institutes
    )
  end
  ...
  -- top-level list.
  local author_insts = flatten(authors:map(function(x) return x.affiliation end))
  ...
  for _, author in ipairs(authors) do
    author.affiliation = pandoc.MetaList(author.affiliation:map(to_index))
  end
...
return {
  {
    Meta = function(meta)
      meta.author, meta.affiliation = canonicalize(meta.author, meta.affiliation)
      return meta
    end
  }
}