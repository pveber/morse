...
This \texttt{R} compagnon provides several tools for ecotoxicologists and regulators based on advanced and innovative methods for a valuable quantitative environmental risk assessment.
...
This \texttt{R} compagnon also allows the analysis of bioassay survival data at the final time. Such data are commonly used to estimate Lethal Concentration ($LC_{x}$) values from chronic toxicity tests. The aim is to fit an exposure-response curve to survival data by Bayesian inference.
