...
```{r, cache=TRUE}
plot(fit_cstSD, facet.label = "conc")
```
...
To have a summary of the estimation of parameters
...
```{r, cache=TRUE}
plot(fit_cstIT, facet.label = "conc")
```
...
```{r, cache=TRUE}
# create a prediction
predict_fit_varSD <- predict(fit_varSD)

#plot the prediction
plot(predict_fit_varSD)
```
