...
# (6) fit a TKTD model
fit.varSD <- survFit(dat_var, quiet = FALSE, model_type = "SD")
# save(fit.varSD, file = "fit.varSD.Rdata")
```

```{r, cache=TRUE}
load("fit.varSD.Rdata")
```
...
```{r, cache=TRUE}
plot(fit.varSD, data_type = "rate")
```

Or the number of survivors with the option `data_type = 'number'`

```{r, cache=TRUE}
plot(fit.varSD, data_type = "number")
```
...
```{r, cache=TRUE, eval = FALSE}
predict_survFitVarC <- predict(fit.varSD)
...
newdata_varC = data.frame(time = seq(0,10,length = 100),
                          conc = c(rep(30,20), rep(0,80)),
                          replicate = rep("new", 100))
...
```{r, cache=TRUE, eval = FALSE}
predict_survFitVarC_newdata <- predict(fit.varSD, newdata = newdata_varC, only.newdata = FALSE)
...
plot(predict_survFitVarC_newdata , adddata = TRUE)
```
