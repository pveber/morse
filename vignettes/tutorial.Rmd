...
Here is a typical session to analyse dose-dependent time-course data
...
For *Stochastic Death* model, we have to specified the `model_type` as `SD`:
...
Then, the function `summary` provides a summary of parameters estimates, medians and their 95 \% credible intervals.

```{r, cache=TRUE}
summary(fit_cstSD)
```

The function `plot` provides a representation of the fitting for each replicates

```{r, cache=TRUE}
plot(fit_cstSD)
```

Original data can be plot by using the option `adddata = TRUE`

```{r, cache=TRUE}
plot(fit_cstSD, adddata = TRUE)
```

A posterior predictive check is also possible using the function `ppc`:

```{r, cache=TRUE}
ppc(fit_cstSD)
```

Compared to arget time analysis, TKTD modelling allows to compute and plot the Lethal Concentration for any percentage of population and at any time.
When time is not specified in `time_LCx`, then the maximal time found in the dataset is used.
...
Warning messages are returned when the range of concentration is not appropriated with one or more LCx point(s). 
...
### Using an alternative model under constant exposure concentration: model IT

The *Individual Tolerance* (IT) model is a variant of TKTD survival
analysis with *SD*. It can also be used with MORSE as demonstrated here. For *IT* model, we have to specified the `model_type` as `IT`:
...
Here is a typical session fitting an SD and IT model for a dataset
with variable exposure concentration.
...
```{r, eval=FALSE, echo=TRUE}
# (6) fit a TKTD SD model
fit_varSD <- survFit(dat, model_type = "SD")
```
...
related to survival analysis on `reproData` object, we can use `survData` as a
conversion function:
