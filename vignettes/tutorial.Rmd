...
GUTS models can be used to simulate the survival of the organisms using the field exposure patterns. Hopefully, this real exposure profile does not result in any related mortality, but a critical question is to know how far the exposure profile is from adverse effect, that is a "margin of safety".
...
Computing an $MF_x$ is easy with the function `MFx`. It only requires the object `survFit` and the exposure profile, argument `profile` in the function. The chosen percentage of survival reduction is specified with argument `x_MFx`, the default is $10$, and the chosen time-point can be specified with `time_MFx`, by default the maximal time-point in the dataset is used. 
...
MFx_PRZ_cstSD_4pred <- MFx(object = fit_cstSD, profile = data_4prediction)
...
To compare the initial survival rate (i.e., a multiplication factor set to 1), and the optimal multiplication factor to obtain a reduction of $x\%$ of survival (provided with argument `x_MFx`), we can use the option `x_variable =  "Time"`
...
MFx_PRZ_cstSD_4pred_x10 <- MFx(object = fit_cstSD, profile = data_4prediction, x_MFx = 10)
plot(MFx_PRZ_cstSD_4pred_x10, log_scale = TRUE)
```
...
MFx_PRZ_cstIT_4pred <- MFx(object = fit_cstIT, profile = data_4prediction, time_MFx = 4, quiet = TRUE)
