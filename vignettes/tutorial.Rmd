...
```{r, cache=TRUE}
# plot(fit_cstSD, style = "ggplot", adddata = TRUE, one.plot = FALSE)
```

```{r, cache=TRUE}
# ppc(fit_cstSD, style = "ggplot")
```
...
```{r, cache=TRUE}
# plot(fit_cstIT, style = "ggplot", adddata = TRUE, one.plot = FALSE)
```

```{r, cache=TRUE}
# ppc(fit_cstIT, style = "ggplot")
```
...
```{r TKTD 1, cache=TRUE}
# (1) load dataset
data(propiconazole_varC_conc)
propiconazole_varC_conc <- propiconazole_varC_conc %>%
  filter(concType == "var") %>%
  select(- concType)

data(propiconazole_varC_Nsurv)
propiconazole_varC_Nsurv <- propiconazole_varC_Nsurv %>%
  filter(concType == "var") %>%
  select(- concType)

# (2) merge data set into a `survData` object
propiconazole_varC <- survData_join(propiconazole_varC_conc, propiconazole_varC_Nsurv)

# (3) check structure and integrity of the dataset
survDataCheck(propiconazole_varC)

# (4) create a `survData` object
dat_var <- survData(propiconazole_varC)
