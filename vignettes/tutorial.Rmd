...
```{r step 4, results="hide", cache=TRUE, eval =FALSE}
fit_survTT <- survFitTT(dat,
                 target.time = 21,
                 lcx = c(10, 20, 30, 40, 50))
```


```{r step 4a, cache=TRUE}
# save(fit_survTT, file = "vignettes/fit_survTT.Rdata")
load("fit_survTT.Rdata")
``` 
...
```{r step 4c, cache=TRUE}
plot(fit_survTT, log.scale = TRUE, style = "ggplot", adddata = TRUE,
     addlegend = TRUE)
```
...
```{r step 4d, results="hide", cache=TRUE}
data("cadmium1")
```

```{r step 4e, eval=FALSE}
wrong_fit <- survFitTT(survData(cadmium1),
                       target.time = 21,
                       lcx = c(10, 20, 30, 40, 50))
```

```{r step 4f, results="hide", cache=TRUE}
# save(wrong_fit, file = "vignettes/wrong_fit_survTT.Rdata")
load("wrong_fit_survTT.Rdata")
...
```{r step 5,cache=TRUE, results="hide"}
ppc(fit_survTT, style = "ggplot")
```
...
```{r surv 2, cache=TRUE, eval=FALSE}
# (6) fit a TKTD model
fit_survTKTD <- survFitTKTD(dat, quiet = TRUE)
```


```{r surv 3, cache=TRUE}
# save(fit_survTKTD, file = "vignettes/fit_survTKTD.Rdata")
load("fit_survTKTD.Rdata")

summary(fit_survTKTD)

plot(fit_survTKTD, style = "ggplot", adddata = TRUE, one.plot = FALSE)
```

```{r surv 4, cache=TRUE}
ppc(fit_survTKTD, style = "ggplot")
```
...
```{r repro 2, cache=TRUE , eval=FALSE}
