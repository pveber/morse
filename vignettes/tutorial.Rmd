...
is `TRUE` the datapoints for a given concentration are pooled
...
The method `plotDoseResponse` can be used to represent the survival rate as a
fonction of concentration for a fixed time. In the case the replicats are poolled
and the confidence interval for each concentration is drawing.
...
# (5) represent the reproduction rate as a function concentration for a fixed time
plotDoseResponse(dat, style = "ggplot")
