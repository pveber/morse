...
# (6) fit a TKTD model
fit_cstSD <- survFitTKTD(dat, quiet = TRUE)
summary(fit_cstSD)
```
...
## Using an alternative model under constant exposure concentration

The "Individual Tolerance" (IT) model is a variant of TKTD survival
analysis. It can also be used with MORSE as demonstrated here:

```{r TKTDcstIT, cache=TRUE}
