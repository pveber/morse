...
Now the dataset with reproduction data at target time is created it is possible
...
The median and 2.5 \% and 97.5 \% of priors and posteriors on estimated parameter distributions and on $LC_{x}$ estimates are given by the `summary()`. The function `plot()` draws the exposure-response curve with the 95 \% confidente interval on observed data.
