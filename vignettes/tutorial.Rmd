...
```{r, cache = TRUE, eval=FALSE}
# create a prediction
predict_survFitCstC <- predict(fit_cstSD)
# save(predict_survFitCstC, file = "predict_survFitCstC.Rdata")
```


```{r, cache=TRUE}
load(file = "predict_survFitCstC.Rdata")
#plot the prediction
plot(predict_survFitCstC, adddata = TRUE)
```
...
```{r}
# The new dataset
data_4casting <- data.frame(time = rep(0:4,3),
                            conc = rep(c(16,25,40), rep(5,3)),
                            replicate = c(rep("16_newdata",5),rep("25_newdata",5),rep("40_newdata",5)))
```


```{r, cache=TRUE, eval=FALSE}
# create a prediction
predict_survFitCstC_Newdata <- predict(object = fit_cstSD, newdata = data_4casting)
# save(predict_survFitCstC_Newdata, file = "predict_survFitCstC_Newdata.Rdata")                             
```


```{r, cache=TRUE}
load(file = "predict_survFitCstC_Newdata.Rdata")
#plot the prediction
plot(predict_survFitCstC_Newdata, adddata = TRUE, 
     levels_vector = c("0", "8", "12", "14", "16_newdata", "18", "24", "25_newdata", "29", "36", "40_newdata"))
```
...
```{r}
# create a prediction
predict_survFitCstIT <- predict(fit_cstIT)
# save(predict_survFitCstIT, file = "predict_survFitCstIT.Rdata")
```


```{r, cache=TRUE}
load(file = "predict_survFitCstIT.Rdata")
#plot the prediction
plot(predict_survFitCstIT,
     levels_vector = c("0", "8", "12", "14", "18", "24", "29", "36"),
     adddata = TRUE)
```
...
# save(fit_varSD, file = "fit_varSD_2.Rdata")
```

```{r, cache=TRUE}
load("fit_varSD_2.Rdata")
```
...
And a prediction of the intermediate data

```{r, cache=TRUE, eval = FALSE}
predict_survFitVarC <- predict(fit_varSD)
# save(predict_survFitVarC, file = "predict_survFitVarC.Rdata")
```

```{r, cache=TRUE}
load(file = "predict_survFitVarC.Rdata")
plot(predict_survFitVarC, adddata = TRUE)
```

And we can forcast on totally different profile

```{r, cache=TRUE}
# newdata_varC = data.frame(time = seq(0,10,length = 100),
#                           conc = 2 + abs(rnorm(100,0,15)),
#                           replicate = rep("newdata", 100))
# save(newdata_varC, file = "newdata_varC.Rdata")
load(file ="newdata_varC.Rdata")

qplot(data = newdata_varC, x = time, y = conc, geom = "line", color = "orange") +
  theme_minimal() + theme(legend.position = "none")
```


```{r, cache=TRUE, eval = FALSE}
predict_survFitVarC_newdata <- predict(fit_varSD, newdata = newdata_varC )
# save(predict_survFitVarC_newdata, file ="predict_survFitVarC_newdata.Rdata")
```


```{r}
load(file ="predict_survFitVarC_newdata.Rdata")
plot(predict_survFitVarC_newdata , adddata = TRUE, keep_level = TRUE)
```
...
```{r, cache=TRUE, eval = FALSE}
predict_survFitVarC_IT <- predict(fit_varIT)
# save(predict_survFitVarC_IT, file = "predict_survFitVarC_IT.Rdata")
```

```{r, cache=TRUE}
load(file = "predict_survFitVarC_IT.Rdata")
plot(predict_survFitVarC_IT, adddata = TRUE)
```
