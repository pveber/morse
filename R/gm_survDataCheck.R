...
gm_survDataCheck <- function(data_surv, data_conc) {
  ...
  ##
    ...
    ### 1.2 data_conc
    ref.namesConc <- c("profile","replicate","time","conc")
  
    if ( ncol(data_conc) != 4  # check same number of column
         | !(all(colnames(data_conc) %in% ref.namesConc))  ) { # check if the name are the same, order doesn't matter
      stop( "'data_conc' must have 4 columns with names: 'profile','replicate','time' and 'conc' (order does not matter)." )
    }
  ...
  if(all(df_check0Surv$check_0) != TRUE || all(df_check0Conc$check_0) !=TRUE ){
    stop( "In each '(profile, replicate)', the first time point must be '0'." )
  }
  ...
  if (!is.numeric(data_conc$conc) | any(data_conc$conc < 0)
      | !is.numeric(data_conc$time) | any(data_surv$conc < 0)      ) {
  ...
  if( all( df_checkReplicate_Nsurv$check_Replicate == df_checkReplicate_conc$check_Replicate ) != TRUE){
...
df = data.frame(a=c(1,1,1), b=c(2,1,3))

df %>% summarize(ch = length(unique(a)))
