...
#' provided as two \code{data.frame}s. The resulting object
...
#' expected requirements. Please run \code{\link{gm_survDataCheck}} to ensure
#' \code{data} is well-formed.
#'
#' @param data_surv a \code{data.frame} containing the following four columns:
#' \itemize{
#' #' \item \code{profile}: a vector of class \code{integer}, \code{character} or \code{factor} for profile
#' identification
...
#' alive individuals at each time point for each profile and each replicate
#' }
...
#' (positive values) at each time point for each profile and each replicate
#' \item \code{time}: a vector of class \code{integer} with time points, min value must be 0
#' }
#'  
#' 
...
gm_survData = function(data_surv, data_conc){
  ...
  df.survData = full_join(data_surv,
                          data_conc,
                          by=c("profile","replicate", "time")) %>%
